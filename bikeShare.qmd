...
# This chunk helps when reusing and are messing with parameters so you don't have to redownload or refilter everything if the filtered table already exist. So you can just connect to the filtered tables quickly. 
...
ttime_mem_Rtype2 <- ttime_mem_Rtype |>
    dplyr::ungroup()
```
...
monthTbl <- dplyr::tbl(dbconn,
    ...
    dplyr::summarize(Freq = dplyr::count()) |>
    dplyr::collect()

monthTbl
summaryMonths <- summary(monthTbl)
summaryMonths
```
...
df <- dplyr::tbl(dbconn,
    ...
    dplyr::summarise(n = dplyr::count()) |>
    dplyr::arrange(start_station_name, start_station_id) |>
    dplyr::collect()
```
