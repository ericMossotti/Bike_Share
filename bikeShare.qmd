...
# Helpful to create a persistent database file. 
dbconn <- DBI::dbConnect(duckdb::duckdb(),
                         dbdir = "db_files/fltrd_data.db",
                         read_only = FALSE)

tripset |>
    as.data.frame() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "db_files/fltrd_data.db",
                         overwrite = TRUE)

duckdb::dbListTables(dbconn)

dplyr::tbl(dbconn,
           "db_files/fltrd_data.db")

db_path <- "db_files/fltrd_data.db"
...
hoursAnimate <- ggplot2::ggplot(data = hours_of_Riders,
    ...
    gganimate::view_step(pause_length = 5,
                         )
...
#unlink("duck_fltr", recursive = TRUE)
```
