...
hour_tbl |>
dplyr::select(hour, member_casual, n) |>
tabler(
title = "Hour Groups",
...
dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(h, member_casual) |>
#dplyr::mutate(h = round(miles, digits = 1)) |>
dplyr::collect() |>
duckdb::dbWriteTable(
conn = dbconn,
name = "db/rounded_h.db",
overwrite = TRUE)
...
dbconn, 
"db/rounded_h.db", 
select_cols = c("h", "member_casual"), 
group_cols = c("h", "member_casual"), 
binary_col = "member_casual", 
ntile_col = "ntile_col", 
pred_col = "h", 
zero_val = "casual", 
...
regression_tbl <- model |>
gtsummary::tbl_regression(
label = list(ntile_col = "Hour Ranges"), 
conf.int = FALSE, 
exponentiate = TRUE)
...
dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(h_m, member_casual) |>
#dplyr::mutate(h = round(miles, digits = 1)) |>
dplyr::collect() |>
duckdb::dbWriteTable(
conn = dbconn,
name = "db/rounded_h_m.db",
overwrite = TRUE)
...
dbconn, 
"db/rounded_h_m.db", 
select_cols = c("h_m", "member_casual"), 
group_cols = c("h_m", "member_casual"), 
binary_col = "member_casual", 
ntile_col = "ntile_col", 
pred_col = "h_m", 
zero_val = "casual", 
...
dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(h_m_s, member_casual) |>
#dplyr::mutate(h = round(miles, digits = 1)) |>
dplyr::collect() |>
duckdb::dbWriteTable(
conn = dbconn,
name = "db/rounded_h_m_s.db",
overwrite = TRUE)
...
dbconn, 
"db/rounded_h_m_s.db", 
select_cols = c("h_m_s", "member_casual"), 
group_cols = c("h_m_s", "member_casual"), 
binary_col = "member_casual", 
ntile_col = "ntile_col", 
pred_col = "h_m_s", 
zero_val = "casual", 
...
```{r}
#| label: axis breaks for casuals' hour 
#| code-summary: Determining optimal breaks in the hour charts based on the x-axis metrics.

source("Scripts/calcBreaks.R")

plotBreaks <- expanded_data |>
dplyr::filter(member_casual == "casual") |>
dplyr::mutate(h_m_s = strptime(h_m_s, format = "%T")) |>
calcBreaks()
...
```{r}
#| label: axis breaks for members' hour 
#| code-summary: Determining optimal breaks in the hour charts based on the x-axis metrics.
...
```{r}
#| label: duration weighted stats modeling
...
:::p-3
```{r}
#| label: speed weighted stats modeling
#| code-summary: "The basic query for running stats on the various parameters. This version returns aggregated distinct rows for running models that utilize weights. Aggregated rows allows the models to run faster and requires less memory, it seems."
...
regression_tbl <- model |>
gtsummary::tbl_regression(
label = list(ntile_col = "MPH Ranges"), 
conf.int = FALSE, 
exponentiate = TRUE)
...
response = unrounded_data$member_casual, 
