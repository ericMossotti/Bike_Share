...
::: section
::: header-container
::: {.heading-box .fs-2 .text-wrap}
About Memberships
:::

::: {.callout-tip .calloutTip icon="false"}
...
::: {.callout-tip .calloutTip .shadow-lg icon="false"}
...
::: fs-3
Can the hour of the day predict membership?
:::
...
hour_tbl <- dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(started_at, member_casual) |>
dplyr::mutate("hour" = lubridate::hour(started_at)) |>
dplyr::arrange(hour) |>
dplyr::collect() |>
...
distance_factored <- dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(miles, member_casual) |>
dplyr::collect() |>
dplyr::arrange(miles, member_casual) |>
dplyr::mutate(
member_casual = forcats::as_factor(member_casual),
member_casual = forcats::fct_inorder(member_casual)
)



distance_factored$member_casual <- stringr::str_replace(distance_factored$member_casual, pattern = "casual", "0")

distance_factored$member_casual <- stringr::str_replace(distance_factored$member_casual, pattern = "member", "1")

distance_factored$member_casual <- distance_factored$member_casual |> as.integer()

distance_factored$member_casual <- factor(
distance_factored$member_casual,
levels = c(0, 1),
labels = c("casual", "member")
)

distance_factored <- distance_factored |> 
dplyr::mutate(
member_casual = forcats::fct_inorder(member_casual)
)
...
plotBreaks <- distance_factored |>
dplyr::filter(member_casual == "casual", miles <= 9) |>
calcBreaks()
```
...
plotBreaks <- distance_factored |>
dplyr::filter(member_casual == "member", miles <= 9) |>
calcBreaks()
```
...
label = list(miles ~ "Speed (miles per hour)"),
digits = list(
miles ~ c(2, 2)),
statistic = 
...
milesModel <- stats::glm(member_casual ~ miles, data = distance_factored, family = binomial)

mileStats_tbl <- milesModel |>
gtsummary::tbl_regression(label = list(miles = "Duration"), conf.int = FALSE)
...
# Generate ROC curve
roc_miles <- pROC::roc(distance_factored$member_casual, predict(milesModel, type = "response"))
...
mph_factored <- dplyr::tbl(dbconn, tblPath_fltrd) |>
...
mph_factored$member_casual <- stringr::str_replace(mph_factored$member_casual, pattern = "casual", "0")

mph_factored$member_casual <- stringr::str_replace(mph_factored$member_casual, pattern = "member", "1")

mph_factored$member_casual <- mph_factored$member_casual |> as.integer()

mph_factored$member_casual <- factor(
mph_factored$member_casual,
levels = c(0, 1),
labels = c("casual", "member")
)

mph_factored <- mph_factored |> 
dplyr::mutate(
member_casual = forcats::fct_inorder(member_casual)
)
```
...
mostFreq <- mph_factored |>
dplyr::filter(member_casual == "member") |>
dplyr::mutate(mph = round(mph, digits = 0)) |>
dplyr::count(mph, name = "count") |>
...
mphModel <- stats::glm(member_casual ~ mph, data = mph_factored, family = binomial)

mphStats_tbl <- mphModel |>
gtsummary::tbl_regression(label = list(mph = "Speed"), conf.int = FALSE)
...
# Generate ROC curve
roc_mph <- pROC::roc(mph_factored$member_casual, predict(mphModel, type = "response"))
...
::: {.callout-tip .calloutTip icon="false"}
-   As evident from @tbl-triptimeTotals and @fig-triptimeTotals, the duration of most trips falls within the range of 5 to 15 minutes.

<!-- -->

-   The group comparison in @tbl-triptimeCompare and @fig-triptimeCompare reveals that both casual riders and members largely follow the overall pattern observed in the totals. However, beyond the 42-minute mark, the count of casual riders exceeds that of members. While members ride more frequently overall, casual riders are more likely to undertake longer trips.


-   Given that $\beta_1 < 0$ (@tbl-logTimes), the results indicate that members are more likely to take shorter trips in terms of duration compared to casual riders.
:::
:::
:::
...
::: fs-3
Trip duration has weak predictive value on membership status
:::

::: p-3
```{r}

duration_factored <- dplyr::tbl(dbconn, tblPath_fltrd) |>
...
duration_factored$member_casual <- stringr::str_replace(duration_factored$member_casual, pattern = "casual", "0")

duration_factored$member_casual <- stringr::str_replace(duration_factored$member_casual, pattern = "member", "1")

duration_factored$member_casual <- duration_factored$member_casual |> as.integer()

duration_factored$member_casual <- factor(
duration_factored$member_casual,
levels = c(0, 1),
labels = c("casual", "member")
)

duration_factored <- duration_factored |> 
dplyr::mutate(
member_casual = forcats::fct_inorder(member_casual)
)
```
...
durationModel <- stats::glm(
member_casual ~ trip_time, data = duration_factored, family = binomial)

durationStats_tbl <- durationModel |>
gtsummary::tbl_regression(
label = list(trip_time = "Duration"), conf.int = FALSE)
...
response = duration_factored$member_casual, 
predictor = predict(durationModel, type = "response"))
...
dbconn,
tblPath_fltrd,
select_cols = c("trip_time", "mph", "miles", "rideable_type", "member_casual"),
group_cols = c("trip_time", "mph", "miles", "rideable_type", "member_casual"),
binary_col = "member_casual",
zero_val = "casual",
one_val = "member"
) |>
dplyr::mutate(
trip_time = round(trip_time, 0),
mph = round(mph, 0),
miles = round(miles, 1),
)
...
model <- weighted_data |> 
glm(
formula = member_casual ~ trip_time, weights = n, family = binomial)

summary(model)

regression_tbl <- model |>
gtsummary::tbl_regression(
label = list(trip_time = "Time"), 
conf.int = FALSE, exponentiate = TRUE)

regression_tbl |>
tabler(
title = gt::md("Binary Logistic Regression: <br> Time & Rider Type"),
source_note = gt::md("**Source**: `db/fltrd_data.db`"),
isBinary = TRUE)
...
# For the trip times comparison
statsi <- dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(member_casual, trip_time) |>
dplyr::mutate(
trip_time = round(trip_time, digits = 0)) |>
dplyr::collect() 

IQR(statsi$trip_time)

summary(statsi$trip_time)
#   25%  ,   25%  ,    25%     25%
# 1<=x<=6, 6<x<=10, 10<x<=16, 16<x<=475  
# 
quantile(statsi$trip_time)

fivenum(statsi$trip_time)
...
# For the trip times comparison
statsi <- dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(member_casual, mph) |>
dplyr::mutate(
mph = round(mph, digits = 0)) |>
dplyr::collect() 
...
dplyr::select(member_casual, miles) |>
dplyr::mutate(
miles = round(miles, digits = 1),
ntiles = dplyr::ntile(miles, n = 4)) |>
dplyr::collect() 

IQR(statsi$miles)

summary(statsi$miles)
...
quantile(statsi$miles)

fivenum(statsi$miles)

model <- statsi |> 
glm(
formula = member_casual ~ miles, family = binomial)
...
tile <-statsi |> dplyr::ntile(statsi$miles)
...
```{r}
#| label: main stats query with weights
...
```{r}
#| label: day of week modeling
#| code-summary: "Predicting the log-odds of being a member versus being a casual user based on the day of the week."

model <- weighted_data |> 
glm(
formula = member_casual ~ wkday, weights = n, family = binomial)
...
regression_tbl <- model |>
gtsummary::tbl_regression(
label = list(wkday = "Weekday"), 
conf.int = FALSE, exponentiate = TRUE)

regression_tbl |>
tabler(
title = gt::md("Binary Logistic Regression: <br> Weekday & Rider Type"),
source_note = gt::md("**Source**: `db/fltrd_data.db`"),
isBinary = TRUE)
...
# Generate ROC curve
roc_continuous <- pROC::roc(
response = freq_data$member_casual, 
predictor = predict(model, type = "response"))

# Find the area under the curve value
auc_continuous <- round(pROC::auc(roc_continuous), 4)

gplot <- roc_continuous |>
plotter(
x_col = NULL, 
y_col = NULL, 
isROC = TRUE,
geomType = "ROC",
title = paste0("ROC Curve for Duration", " (AUC = ", auc_continuous, ")"),
x_label = paste0("Specificity", "\n", "(False Positive Rate)"), 
y_label = paste0("Sensitivity", "\n", "(True Positive Rate)"),
roc_color = "red")
...
For tree-based algorithms, multicollinearity wouldn't matter much as they split on the feature that gives higher information gain.

VIF = 1/(1-R^2)


### Interpretation of Contingency Table Data
...
::: {.accordion-button .collapsed type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne" style="background-color: #6D4C41"}
...
::: {.accordion-button .collapsed type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo" style="background-color: #5D4037"}
...
::: {.accordion-button .collapsed type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree" style="background-color: #4E342E"}
...
::: {.accordion-button .collapsed type="button" data-bs-toggle="collapse" data-bs-target="#collapseFour" aria-expanded="false" aria-controls="collapseFour" style="background-color: #3E2723"}
