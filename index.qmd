...
abstract: "Communicating reproducible, data-driven insight into the questions relating but not limited to, increasing annual membership subscriptions."
...
```{r}
#| label: tbl-raw
#| tbl-cap: "Unaltered data (other than being combined and formatted for presentation)."
#| cap-location: top
#| tidy: true
...
tabler(
title = "What data did we start with?",
...
:::{.d-flex .justify-content-center .tableScroller}
```{r}
#| label: tbl-dbList
#| code-summary: "These are the starting tables contained in the data/data.db file.<br><br>Noting this as we will be adding many more tables in the later stages."
#| tbl-cap: "The list of tables created in data.db so far."
#| tidy: true

dbList <- duckdb::dbListTables(dbconn) |>
as.data.frame() |>
tabler(
title = gt::md("Which tables have<br>been created so far?"),
...
label_n = NULL,
align_parameter = "left"
) |>
gt::cols_label("duckdb::dbListTables(dbconn)" = "Paths") |>
...
:::{.column-margin}
![dbList_1](images/dbFiles_start.png){#fig-dbList_1}
...
::: p-2
The starting occurrences was 5,719,877. Then 1,388,170 incomplete observations were then removed by the initial processing script.
:::
...
-   @tbl-memberTotals and @fig-totalMemberFrequency give an idea of the overall member to casual travel distribution.
:::
...
::::: panel-tabset
### [Overall]{.panel-tabset-label}

::: plot-container
```{r}
#| label: fig-totalMemberFrequency
#| fig-cap: "Plotting the overall membership distribution"
#| tidy: true
#| echo: true
...
y_col = n, 
geomType = "column", 
title = "How often do members travel?",
...
-   @tbl-btypeTotal and @fig-btypeTotal present the overall frequency distribution for choice of bicycle.

-   Likewise, @tbl-btypeGroups and @fig-btypeGroups summarize the choice of bicycle with the addtion of a grouped sum by the membership status.

-   @tbl-btypeChiSquare presents a Chi-Square analysis of bicycle type usage among casual users and members.
...
-   @tbl-btypeModel presents the results of a binary logistic regression analyzing the relationship between bicycle type and membership status. The analysis compares classic bikes and electric bikes, with classic bikes serving as the reference category.
...
#| fig-cap: "The overall distribution of trips taken by bicycle choice"
#| tidy: true
...
y_col = n, 
geomType = "column", 
title = paste0("Which bicycles are preferred?"),
x_label = "Type", 
y_label = "n"
)
...
#| tbl-cap: "The overall distribution of bicycle choice"
#| tbl-cap-location: top
#| tidy: true
...
tabler( 
title = NULL, 
note_list = list("bicycle type","occurrences"),
...
### [Comparative]{.panel-tabset-label}

:::: panel-tabset
#### [Frequency]{.panel-tabset-label}

```{r}
#| label: fig-btypeGroups
#| fig-cap: "The membership distribution of bicycle choice"
#| tidy: true
...
plotter(
title = paste0("Which bicycles are members choosing?"),
x_label = "Type",
y_label = "n",
x_col = rideable_type, 
y_col = n, 
...
::: {.tbl-container .column-margin}
```{r}
#| label: tbl-btypeGroups
#| tbl-cap: "The membership<br>distribution of bicycle choice"
#| tbl-cap-location: top
#| tidy: true
#| echo: false
...
#| tbl-cap: "Testing indepdence of bicycle choice and membership variables"
#| tidy: true

chi_table <- tabler(
title = gt::md("Chi-Square:<br>The signficance of bicycle choice and membership"),
source_note = gt::md("**Source**: `db/bType.db`"),
label = list(
...
```{r}
#| label: btypeModel
#| code-summary: "Predicting the log-odds of being a member versus being a casual user."
#| tidy: true
...
#| tbl-cap: "Modeling the probability of member travel by the choice of bicycle."
#| tidy: true

regression_tbl <- model |>
gtsummary::tbl_regression(
label = list(rideable_type = "Bicycle Choice"), conf.int = FALSE, exponentiate = TRUE)

regression_tbl |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of members' bicycle choices"),
...
-   @tbl-triptimeTotals and @fig-triptimeTotals give an idea of the overall travel duration distribution. The data were rounded to the nearest minute for these figures.

-   Likewise, @tbl-triptimeCompare and @fig-triptimeCompare summarize the travel duration distribution, where frequencies were summed by membership status.

-   @tbl-durationSummary sheds light on the variability, range, and quartile information about the duration data.

-   To accompany the summary table, two plots, @fig-durationCasualHistogram and @fig-durationMemberHistogram, present a highly granular view of the duration dataset (the solid yellow line represents the mean). Quartile ranges are likewise shown to help understand the variability.

-   Supplementing the histogram, @fig-durationDensity shows a density plot comparing the duration of trips for ***casual*** users and ***members***. The x-axis represents time in minutes, limited to a range of 0 to 100 for presentation purposes, while the y-axis shows the density (a measure of relative frequency).

    -   Both groups exhibit right-skewed distributions, with a peak near the left side and a long tail extending to the right. This suggests that for both groups, a shorter duration is more common, while a longer duration occurs less frequently but can extend quite far.

    -   The **member** group (darker blue) has a higher and narrower peak compared to the **casual** group (lighter blue). This indicates that members tend to have more concentrated distribution of a session duration around their most common length.
    ...
    -   There's significant overlap between the two distributions, indicating that while there are differences, there is also considerable similarity in duration patterns between casual users and members.

-   @tbl-modeldurationQ presents the results of a binary logistic regression model, analyzing the relationship between ride duration and membership status. The analysis divides ride duration into quartiles, with ***Q1 (1.02 - 5.73 minutes)*** serving as the reference category.
...
#| code-summary: Query, transform, and write weighted quartile data. 
#| tidy: true
...
#| fig-cap: "The overall distribution of trip durations"
#| tidy: true
...
y_col = n,
geomType = "column", 
title = paste0("How long are the trips?"),
x_label = "Minutes", 
y_label = "n",
color_col = "black") +
...
#| tbl-cap: "The overall distribution of trip durations"
#| tbl-cap-location: top
#| tidy: true
...
tabler( 
title = NULL, 
note_list = list("trip duration", "occurrences"),
...
#| fig-cap: "The membership distribution of trip durations"
#| tidy: true
...
plotter(
title = paste0("How long do members ride for?"),
x_label = "Minutes",
y_label = "n",
x_col = mins, 
y_col = n, 
...
#| tbl-cap: "The membership distribution of trip durations"
#| tbl-cap-location: top
#| tidy: true
...
#| tbl-cap: "The mean with standard deviation, median with inter-quartile distance, and range with min and max of member travel duration."
#| tidy: false
...
#### [Histogram]{.panel-tabset-label}

```{r}
#| label: durationQuartiles
#| code-summary: "Create a data frame, then extract the desired quartile info to supplement histogram visualization for the data."
#| tidy: true
...
quartiles_member <- 
quantile(qdf_member$trip_time, probs = c(0.25, 0.5, 0.75))

quartiles_casual <- 
quantile(qdf_casual$trip_time, probs = c(0.25, 0.5, 0.75))

```

```{r}
#| label: fig-durationCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of casual travel duration."
#| tidy: true
...
```{r}
#| label: fig-durationMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of member travel duration."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution between duration and membership variables."
#| tidy: true
...
select_cols = c("trip_time", "member_casual")) |>
plotter(
title = "Visualizing relative frequency\n across travel duration groups",
...
#| code-summary: Query, process, and create model R object for hour based on quartile range.
#| tidy: true
...
#| tbl-cap: "Modeling the probability of members' travel by the duration."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of members' travel durations"),
...
-   @tbl-monthTotals and @fig-monthTotals shows the overall monthly travel distribution.

-   Similarly, @tbl-monthCompare and @fig-monthCompare summarize the monthly travel, where frequencies were summed by membership status.

-   In @tbl-chiMonths presentes a Chi-Square of monthly travel among members and casual users.

    -   Members exhibit higher usage during the months of Jan - Mar and Oct - Dec.

    -   Both groups prefer the months of Apr - Sep, where casuals show a higher interest, proportionally for their respective group.
...
-   Two plots, @fig-monthCasualHistogram and @fig-monthMemberHistogram, present a highly granular view of the monthly data (the solid yellow line represents the mean). Quartile ranges are likewise shown to help understand the variability.

-   To visualize monthly users through the lens of their respective concentrations, see @fig-monthDensity. The plot looks a little different because date-time data was used directly when plotting the x-axis. The y-axis shows the density (a measure of relative frequency).

    -   The casual group has higher peaks near the middle of the distribution.

    -   The member group has higher peaks near the left and right sides of the distribution.

    -   Both of the groups exhibit somewhat normal, multi-modal distributions.

    -   Both groups overlap the other signficantly, suggesting similarity in travel patterns.

-   @tbl-modelMonthsQ presents the results of a binary logistic regression analyzing the relationship between months of the year and membership status. The analysis divides the year into quartiles, with ***Q1 (January 01 - May 20)*** serving as the reference category.
...
#| fig-cap: "The overall distribution of trips taken by month"
#| tidy: true
...
y_col = n,
geomType = "column", 
title = paste0("Which months do people tend to ride?"),
...
#| tbl-cap: "The overall distribution of trips taken by month"
#| tbl-cap-location: top
#| tidy: true
...
tabler(
title = NULL,
note_list = list("month of year", "occurrences"),
...
#| fig-cap: "The membership distribution of trips taken by month"
#| tidy: true
...
plotter(
title = paste0("Which months do members tend to travel?"), 
x_label = "Months",
y_label = "n",
x_col = abbMonths, 
y_col = n, 
...
#| tbl-cap: "The membership distribution of trips taken by month"
#| tbl-cap-location: top
#| tidy: true
...
```{r}
#| label: tbl-chiMonths
#| tbl-cap: "Testing the independence between month and membership variables"
#| tidy: true

chi_table <- tabler(
title = gt::md("Chi-Square:<br>The signficance of monthly travel on membership"),
source_note = gt::md("**Source**: `db/moy.db`"),
...
quartiles_member <- 
quantile(qdf_member$started_at, probs = c(0.25, 0.5, 0.75))
...
```{r}
#| label: fig-monthCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of monthly casual travel."
#| tidy: true
...
```{r}
#| label: fig-monthMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of monthly member travel."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution of months by membership."
#| tidy: true
...
#| tbl-cap: "Modeling the probability of monthly member travel."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of monthly member travel"),
source_note = gt::md("**Source**: `db/moy.db`"),
isBinary = TRUE)
...
-   @tbl-wkdayTotals and @fig-wkdayTotals show an aggregated distribution of the overall daily travel.

-   Similarly, @tbl-wkdayCompare and @fig-wkdayCompare summarize the duration distribution, grouping sums by membership status.

-   In @tbl-chiDays, present a Chi-Square of daily travel among members and casual users.

    -   Members show higher usage Mon - Thu compared to casual users. The pattern is flatter compared to casual users.

    -   Casual users show higher usage Fri - Sun compared to members. Their preferences fall within the weekend.
...
-   Two plots, @fig-dayCasualHistogram and @fig-dayMemberHistogram, present a highly granular view of the daily data (the solid yellow line represents the mean). Quartile ranges are likewise shown to help visualize the variability.

-   To visualize daily users through the lens of their respective concentrations, see @fig-dayDensity The plot looks a little different because date-time data was used directly when plotting the x-axis. The y-axis shows the density (a measure of relative frequency).

    -   The casual group has higher peaks near the left and right sides of the distribution. Their highest peak appears to be on Saturday.

    -   The member group has higher peaks near the middle of the distribution. They also show less day-to-day variability during weekdays compared to casual users.

    -   On weekdays, both groups show a bimodal distribution, with two peaks each day. On weekends, there is a unimodal distribution.

    -   Both groups overlap the other signficantly, suggesting similarity in travel patterns.

-   @tbl-modelDaysQ presents the results of a binary logistic regression analyzing the relationship between days of the week and membership status. The analysis divides the week into quartiles, with Q1 (Sunday 12:00 am - Monday 11:40 am) serving as the reference category.
...
#| fig-cap: "The overall distribution of trips taken by day of the week"
#| tidy: true
...
#| tbl-cap: "The overall distribution of trips taken by day of the week"
#| tbl-cap-location: top
#| tidy: true
...
tabler( 
title = NULL,
note_list = list("day of the week", "occurrences"),
...
#| fig-cap: "The membership distribution of trips taken by day of the week"
#| tidy: true
...
#| tbl-cap: "The membership distribution of trips taken by day of the week"
#| tbl-cap-location: top
#| tidy: true
...
tabler(
title = gt::md("Chi-Square:<br>The signficance of daily travel on membership"),
source_note = gt::md("**Source**: `db/moy.db`"),
...
```{r}
#| label: dayQuartiles
#| code-summary: "Creates a data frame, then extract the desired quartile info to supplement histogram visualization for the data."
#| tidy: true
...
```{r}
#| label: fig-dayCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of daily casual travel."
#| tidy: true
...
```{r}
#| label: fig-dayMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of daily member travel."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution of the day by membership variables."
#| tidy: true
...
```{r}
#| label: modelDaysQ
#| code-summary: Query, process, and create a model object for hour based on quartile range. 
#| tidy: true
...
#| tbl-cap: "Modeling the probability of daily member travel."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of daily member travel"),
...
-   @tbl-hourTotals and @fig-hourTotals give an aggregated distribution of the overall hourly travel distribution.

-   Similarly, @tbl-hourMembership and @fig-hourCompare summarize the distribution, grouping the sums by membership.

-   @tbl-chiHours presents a Chi-Square of hourly travel among members and casual users.

    -   Compared to casual users, members show relatively higher service usage at intervals \[5:30am - 9:30am\] and \[4:30pm - 6:30pm\].

    -   Casual users show relatively higher rates at intervals \[10:30am - 4:30pm\] and \[6:30pm - 5:30am\].
...
-   Two plots, @fig-hourCasualHistogram and @fig-hourMemberHistogram, present a highly granular view of the hourly data (the solid yellow line represents the mean). Quartile ranges are likewise shown to help visualize the variability.

-   To visualize hourly users through the lens of their respective concentrations, see @fig-hourDensity. The plot looks a little different because date-time data was used directly when plotting the x-axis. The y-axis shows the density (a measure of relative frequency).

    -   The member group parallels the daily bimodal patterns seen in daily density plot.

    -   The casual group likewise, but to less extent parallels the bimodal patterns seen in the daily density plot.

    -   The two groups, largely, are overlapping, with the highest densities for both falling around 5:00pm.

-   @tbl-modelHourQ presents the results of a binary logistic regression analyzing the relationship between hour of the day and membership status. The analysis divides the day into quartiles, with Q1 (12:00 am - 10:59 am) serving as the reference category.
...
#| fig-cap: "The overall distribution of trips taken by hour of the day"
#| tidy: true
...
y_col = n,
geomType = "column", 
title = paste0("Which hours do people tend to ride?"),
x_label = "Hour", 
y_label = "n",
) +
...
#| tbl-cap: "The overall distribution of trips taken by hour of the day"
#| tbl-cap-location: top
#| tidy: true
...
tabler( 
title = NULL,
note_list = list("hour of the day", "occurrences"),
...
#| fig-cap: "The membership distribution of trips taken by hour of the day"
#| tidy: true
...
plotter(
title = paste0("Which hours of the day do members ride?"),
x_label = "Hour of Day",
y_label = "n",
x_col = hr, 
y_col = n, 
...
#| tbl-cap: "The membership distribution of trips taken by hour of the day"
#| tbl-cap-location: top
#| tidy: true
...
```{r}
#| label: tbl-chiHours
#| tbl-cap: "Testing the independence between hour and membership variables"
#| tidy: true

tabler(
title = gt::md("Chi-Square:<br>The signficance of hourly travel on membership"),
source_note = gt::md("**Source**: `db/hod.db`"),
label = list(hr = "Hour"),
by = member_casual,
isSummary = TRUE,
...
quartiles_member <- 
quantile(qdf_member$started_at_time, probs = c(0.25, 0.5, 0.75))
...
```{r}
#| label: fig-hourCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of hourly casual travel."
#| tidy: true
...
```{r}
#| label: fig-hourMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of hourly member travel."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution of the hour by membership."
#| tidy: true
...
#| tbl-cap: "Modeling the probability of hourly member travel."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of hourly member travel"),
...
-   @tbl-milesTotals and @fig-milesTotals show the overall distribution of distances traveled.

-   @tbl-milesCompare and @fig-membershipMiles likewise summarize the distance distribution by membership status.

-   @tbl-distanceSummary sheds light on the variability, range, and quartile information about the distance to membership data.

-   Two plots, @fig-distanceCasualHistogram and @fig-distanceMemberHistogram, present a highly granular view of the distance data (the solid yellow line represents the mean). Quartile ranges are likewise shown to help visualize the variability.

-   To visualize monthly users through the lens of their respective concentrations, see @fig-distanceDensity. The y-axis shows the density (a measure of relative frequency).

    -   The member group has a narrower, taller spike in between 0.1 and 1 mile distances. There is less of a concentration than the casual group between the 1-3 mile distances.

    -   The casual group displays a broader spike with much overlap to the member group, but peaks closer to the 1 mile mark than for members. There is greater concentration than members between the 1-3 miles distances, but less around the 0.1-1 mile distance.

    -   Both groups overlap nearly completely from around the 3 miles mark and onwards.
...
#| fig-cap: "The overall distribution of trips taken by the estimated distance traveled"
#| tidy: true
...
y_col = n,
geomType = "column", 
title = paste0("How far do people ride?"),
x_label = "Miles", 
y_label = "n") 
...
#| tbl-cap: "The overall distribution of trips taken by the esimated distance traveled"
#| tbl-cap-location: top
#| tidy: true
...
#| fig-cap: "The membership distribution of trips taken by the estimated distance traveled"
#| tidy: true
...
plotter(
title = paste0("How far do members travel?"),
x_label = "Miles",
y_label = "n",
x_col = milesR, 
y_col = n, 
...
#| tbl-cap: "The membership distribution of trips taken by the estimated distance traveled"
#| tbl-cap-location: top
#| tidy: true
...
#| tbl-cap: "The mean with standard deviation, median with inter-quartile distance, and range with min and max of member travel distance."
#| tidy: false
...
quartiles_member <- 
quantile(as.numeric(qdf_member$miles), probs = c(0.25, 0.5, 0.75))
...
```{r}
#| label: fig-distanceCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of casual travel distance."
#| tidy: true
...
```{r}
#| label: fig-distanceMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of member travel distance."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution of distance by membership."
#| tidy: true
...
#| tbl-cap: "Modeling the probability of members' travel by distance."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of members' travel distance"),
...
-   @tbl-mphTotals and @fig-mphTotals show the overall distribution of travel speeds.

-   @tbl-mphCompare and @fig-mphCompare summarize the travel speed distribution by membership.

-   @tbl-speedSummary gives the reader some idea of the variability, range, and quartile information about the distance data.

-   Two plots, @fig-speedCasualHistogram and @fig-speedMemberHistogram, present a highly granular view of the travel speed data (the solid yellow line represents the mean). Quartile ranges are likewise shown to help visualize the variability.

-   To visualize travel speed patterns through the lens of their respective concentrations, see @fig-speedDensity. The y-axis shows the density (a measure of relative frequency).

    -   The member group has a narrower, taller spike centering around 7mph. There is higher concentration than the casual group between \~ 5-15mph.

    -   The casual group has a broader, shorter spike centering closer to 6mph. There is higher concentration than the member group between \~ 1-5mph.

    -   There is a lot of overlap, and both groups have a unimodal distribution.
...
```{r}
#| label: fig-mphTotals
#| fig-cap: "The overall distribution of trips taken by the estimated average speed"
#| tidy: true
...
y_col = n,
geomType = "column", 
title = paste0("How fast do people ride?"),
...
```{r}
#| label: tbl-mphTotals
#| tbl-cap: "The overall distribution of trips taken by the estimated average speed"
#| tbl-cap-location: top
#| tidy: true
...
tabler(
title = NULL,
note_list = list("travel speed (miles per hour)", "occurrences"),
location_list = list("mphR", "n"),
source_note = gt::md("**Source**: `db/speed.db`"),
noteRows = TRUE,
hide_column_labels = TRUE,
row_index = 1,
label_parameter = "mphR",
align_parameter = "right"
)
...
#| fig-cap: "The membership distribution of trips taken by the estimated average speed"
#| tidy: true
...
plotter(
title = paste0("How fast do members travel?"),
x_label = "Miles per Hour",
y_label = "n",
x_col = mphR, 
y_col = n, 
...
#| tbl-cap: "The membership distribution of trips taken by the estimated average speed"
#| tbl-cap-location: top
#| tidy: true
...
#| tbl-cap: "The mean with standard deviation, median with inter-quartile distance, and range with min and max of member travel speed."
#| tidy: false
...
quartiles_member <- 
quantile(qdf_member$mph, probs = c(0.25, 0.5, 0.75))
...
```{r}
#| label: fig-speedCasualHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of casual travel speed."
#| tidy: true
...
```{r}
#| label: fig-speedMemberHistogram
#| fig-cap: "A more detailed look that illustrates the quartile distribution of member travel speed."
#| tidy: true
...
#| fig-cap: "Visualizing the probability-density distribution of speed by membership."
#| tidy: true
...
#| tbl-cap: "Modeling the probability of members' travel by speed."
#| tidy: true
...
exponentiate = TRUE) |>
tabler(
title = gt::md("Binary Logistic Regression:<br>Modeling the likelihood of members' travel speed"),
...
#| tbl-cap: "The list of tables created by the end of the analysis."
#| tidy: true
...
::: column-margin
![dbList_2](images/dbFiles_end.png){#fig-dbList_2}
