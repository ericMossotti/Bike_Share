...
totalMembers <- dplyr::tbl(dbconn, "db/freq_member.db") |>
dplyr::collect()
...
gt::tab_footnote(
"Total trips taken in 2023 by membership.",
locations = gt::cells_column_labels(columns = "n")
) |>
gt::tab_footnote(
"Membership groups where casuals are those that pay as they go and members pay an annual subscription.",
locations =   gt::cells_column_labels(columns = "member_casual")
)
```
:::
...
memberFrame <- dplyr::tbl(dbconn, "db/freq_member.db") |>
dplyr::collect()

outPlot <- plotter(data = memberFrame, x_col = member_casual, y_col = n, geomType = "column", title = "Membership Types", x_label = "Rider Types", y_label = "Trips")
...
totalTypes <- dplyr::tbl(dbconn, "db/freq_rTypes.db") |>
dplyr::collect()

tabler(totalTypes, title = "Bicycles", subtitle = "2023") |>
gt::cols_label(n = "Trips", rideable_type = "Type") |>
gt::tab_footnote(
"Total trips taken in 2023 according to bicycle type.",
locations =   gt::cells_column_labels(columns = "n")
) |>
gt::tab_footnote(
"Bicycle types, where classic is manually powered and electric can be both manual and electrically powered.",
locations =   gt::cells_column_labels(columns = "rideable_type"))
...
compareTypes <- dplyr::tbl(dbconn, "db/freqCompare_rType.db") |>
dplyr::collect()
...
timeCompare <- dplyr::tbl(dbconn, "db/freqCompare_tripTime.db") |>
dplyr::collect()
