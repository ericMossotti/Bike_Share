...
#| code-summary: This then would be executed if the right if-else conditions were met. Usually, this would only execute if there is no db folder and its associated files.
...
summary(miles_asNums)

# B1 < 0, probability that Y = 1 decreases with X 
# the probability that members ride decreases with trip distance, measured in miles
```
...
# B > 0, so the probability of a higher average trip speed is more likely for members than casuals
```
...
# B < 0, so the probability of a lower trip-time is higher for members than casuals
```
## Testing Frequency Distribution with Chi-Square

```{r}
#| label: chi-square for months

duckdb::dbListTables(dbconn)


months4chi <- dplyr::tbl(dbconn,
           tblPath_fltrd,
           check_from = FALSE) |>
    dplyr::select(member_casual,
                  started_at) |>
...
monthChi_expected <- monthChi$expected
monthChi_observed <- monthChi$observed

monthChi_expected
monthChi_observed

# p < 0.05, evidence that the Null Hypothesis can be rejected,
#  there is a strong chance that there is an association between how likely a rider uses the service during a given month, and what their membership status is
```
