...
```{r}
#| label: mainHoursQuery
#| code-summary: How I created the database table for the purpose of hours related EDA.  
#| tidy: true
...
```{r}
#| label: tbl-dbHours
#| code-summary: A kable output of the hours db table.
#| tidy: formatR

dplyr::tbl(dbconn, "db/base_hours.db") |>
dplyr::collect() |>
head()
#kableExtra::kable()
```
...
dplyr::add_count(hr) |>
dplyr::distinct(hr, member_casual, n) |>
tabler(
...
gplot <- dplyr::tbl(dbconn, "db/base_hours.db") |>
dplyr::collect() |>
plotter(
...
#| code-summary: How I weighted the data by quartile for hours. 
#| tidy: formatR

transformData(
conn = dbconn,
path = "db/base_hours.db",
...
duckdb::dbWriteTable(conn = dbconn, name = "db/hour_wtq.db", overwrite = TRUE)
```
```{r}
#| label: tbl-hoursWTQ
...
dplyr::tbl(dbconn, "db/hour_wtq.db") |>
dplyr::collect() |>
head() |>
kableExtra::kable()
```
...
```{r}
#| label: modelHourQ
#| code-summary: Create model object for hour based on quartile range. 
...
#| code-summary: Model fitting results for hour's quartile ranges. 
...
```{r}
#| label: nHr
#| code-summary: How to grouped n by hour and member was created.

dplyr::tbl(dbconn, "db/base_hours.db") |>
dplyr::collect() |>
dplyr::mutate(
hr = forcats::as_factor(hr),
member_casual = forcats::fct(member_casual, levels = c("casual", "member"))
) |>
dplyr::add_count(member_casual, hr) |>
dplyr::arrange(hr, member_casual) |>
dplyr::distinct(member_casual, hr, n) |>
duckdb::dbWriteTable(conn = dbconn, 
name = "db/hour_nHr.db", 
overwrite = TRUE)
```
...
gplot <- dplyr::tbl(dbconn, "db/hour_nHr.db") |>
dplyr::collect() |>
plotter(
title = "Hour Groups",
x_label = "Hour of Day",
...
dplyr::tbl(dbconn, "db/hour_nHr.db") |>
dplyr::collect() |>
