...
::: p-3
```{r}
#| label: accurate density plot for hour
#| code-summary: "Seems like a more accurate hour query and density plot."


compare_hours <- dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(started_at, member_casual) |>
dplyr::arrange(started_at) |>
dplyr::collect() |>
dplyr::mutate(
...
compare_hours$started_at <- update(
compare_hours$started_at, 
year = 2023, 
month = 1, 
day = 1)
...
x_label = paste0("Hour of the Day"),
#y_label = "Trips",
x_col = started_at, 
group_col = member_casual,
geomType = "other",
...
::: p-3
```{r}
#| label: modeling hours
#| code_summary: Modeling hours

dplyr::tbl(dbconn, tblPath_fltrd) |>
dplyr::select(h_m_s, h, member_casual) |>
#dplyr::mutate(h = round(miles, digits = 1)) |>
dplyr::collect() |>
duckdb::dbWriteTable(
conn = dbconn,
...
dbconn, 
"db/rounded_h_m_s.db", 
select_cols = c("h_m_s", "h", "member_casual"), 
group_cols = c("h_m_s", "h", "member_casual"), 
binary_col = "member_casual", 
ntile_col = "ntile_col", 
pred_col = "h_m_s", 
zero_val = "casual", 
...
tabler(
title = gt::md("Summary Statistics: <br> Hours - Membership"),
source_note = gt::md("**Source**: `db/fltrd_data.db`"),
isBinary = TRUE
)
...
# Generate ROC curve
roc_hours <- pROC::roc(expanded_data$member_casual, expanded_data$h, levels = c("casual", "member"))


# Find the area under the curve value
auc_hours <- round(pROC::auc(roc_hours), 4)
...
::: fs-3
Trip duration's predictive value on membership status
:::
